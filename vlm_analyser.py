...
from utils.retry import compress_frame_for_vlm, compute_frame_similarity
...
def analyse_frame(
    ...
    *,
    ocr_prefetch:  Optional[Tuple[str, List[str]]] = None,
    skip_vlm:      bool = False,
) -> FrameAnalysis:
    ...
    try:
        try:
            analysis.scene_description = _vlm_call(
                ...
                image_b64, cfg.VLM_MAX_TOKENS, cfg.VLM_TEMPERATURE, mime_type)
        except Exception as e:
        ...
        try:
            _parse_slide_json(analysis, _vlm_call(
                ...
                image_b64, 256, cfg.VLM_TEMPERATURE, mime_type))
        except Exception as e:
...
def analyse_all_frames(
    ...
    """
    Process every frame one at a time — strictly sequential, no threads.

    Per frame:
      1. Skip if cached.
      2. Skip (copy prev) if perceptually identical to last analysed frame.
      3. Run OCR.
      4. Adaptive VLM gate: skip VLM if ocr_lines >= max(floor, running_avg).
    ...
    skipped_sim         = 0
    skipped_vlm         = 0
    last_analyzed_path: Optional[Path] = None
